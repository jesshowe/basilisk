...
linearTranslationOneDOFStateEffector::~linearTranslationOneDOFStateEffector()
{
    linearTranslationOneDOFStateEffector::effectorID = 1;    /* reset the panel ID*/
}

void linearTranslationOneDOFStateEffector::Reset(uint64_t CurrentSimNanos) {
    // Normalize the sHat vector
    if (this->fHat_B.norm() > 0.01) {
        this->fHat_B.normalize();
    }
    else {
